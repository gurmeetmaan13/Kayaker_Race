    ...
    public partial class Form1 : Form
    {
            ...
            while (End == false)
            {
                for (var i = 0; i < 4; i++)
                {
                    ...
                    if (mykayaker[i].mypb.Top > 400)
                    {
                        ...
                        lblResult.Text = mykayaker[i].Name + " Won!"; // displays in a label which kayaker won
        ...
        // sets each bettors label to display amount bet & kayaker bet on
        {
            ...
            for (int i = 0; i < currentPunter.Length; i++)
            {
                ...
                currentPunter[i].MyLabel.Text = currentPunter[i].PlayerName +
                                                          " Bet " + currentPunter[i].MyBet.Amount + " On Kayaker " +
                                                          currentPunter[i].MyBet.Kayaker;
            }

        }

        private void btnReset_Click(object sender, EventArgs e) // resets everything so the betting continues untill everyone is bankrupt
        {
            ResetKayakers();
        }
    }
